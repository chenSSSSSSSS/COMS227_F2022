...
public class SimpleTest
{
  public static void main(String[] args)
  {
    ...
    LinesGame game1 = new LinesGame(grid, lines);
    
    // Verify that the constructor and the two simple accessors work
    System.out.println(grid == game1.getGrid());      // expected true
    System.out.println(lines == game1.getAllLines()); // expected true
    ...
    System.out.println(StringUtil.originalGridToString(game1.getGrid()));
    System.out.println(StringUtil.currentGridToString(game1.getGrid()));
    System.out.println(StringUtil.allLinesToString(game1.getAllLines()));
    
    // or, we can accomplish the above by invoking toString on the game 
    System.out.println(game1);
    
    // try a couple of moves
    game1.startLine(1, 3); // (1, 3) is an endpoint for id 4 (Y)  
    
    // now, the game's current line should be the one with id 4
    // and it should have location (1, 3) in its list
    // Expected:
    // id 4(Y): {(2, 0), (1, 3)} [(1, 3)]
    Line line = game1.getCurrentLine();
    System.out.println(line);
   
    // This should also increment the count for the cell at (1, 3) 
    // from 0 to 1
    GridCell gc = game1.getCell(1, 3);
...
//    game1.addCell(1, 0);
    ...
    GridCell[][] first = StringUtil.createGridFromStringArray(test);
    
    ArrayList <Line> lineee = Util.createLinesFromGrid(first);
  
    System.out.println(lineee.get(0).getId());
