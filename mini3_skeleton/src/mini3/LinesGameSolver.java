...
public class LinesGameSolver
{
  ...
  {
    // TODO
	  if(g.getCurrentLine().isConnected())
	  {
		  if(g.getCurrentLine().equals(g.getAllLines().get(g.getAllLines().size()-1)))
		  {
			  if(g.isComplete())
			  {
				  solutions.add(new Solution(g));
			  }
			  else
			  {
				  return;
		  ...
		  {
			  int idx = g.getAllLines().indexOf(g.getCurrentLine());
			  g.endLine();
			  Line l = g.getAllLines().get(idx + 1);
			  g.startLine(l.getEndpoint(0).row(), l.getEndpoint(0).col());
			  
			  findSolutions(g, solutions); //? find solution for which one 
			  g.undoMove();  //?
		  }
	  }
	  else
	  {
		  Location last = g.getCurrentLine().getLast();
		  
		  int row1 = last.row();
		  int col1 = last.col();
		  
		  Location [] s = getNeighbors (row1, col1);
		  ...
		  {  
			  ...
			  int countMoves = g.getMoveCount();
			  
			  g.addCell(row2, col2);
			  
			  if(countMoves + 1 == g.getMoveCount())
			  {
				  findSolutions(g, solutions);
				  g.undoMove();
			  }
		  }
